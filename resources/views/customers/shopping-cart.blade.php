		...
		<div class="container-fluid">
			<div class="row">
							...
							<table class="table-shopping-cart">
								...
								@foreach ($keranjangAda as $cart)
									@php
										...
										$variantText = '';
										$selectedVariantValueIds = $cart->keranjang_variant->pluck('produk_variant_value.id_variant_value')->sort()->values();

										if ($cart->keranjang_variant->isNotEmpty()) {
											$variantText = $cart->keranjang_variant->map(function ($v) {
												return $v->produk_variant_value->variantAttribute->nama_variant . ': ' . $v->produk_variant_value->variantValues->value;
											})->implode(', ');
										}
								...
								@foreach ($keranjangHabis as $item)
									@php
										...
										if ($item->keranjang_variant->isNotEmpty()) {
											$variantText = $item->keranjang_variant->map(function ($v) {
												return $v->produk_variant_value->variantAttribute->nama_variant . ': ' . $v->produk_variant_value->variantValues->value;
											})->implode(', ');
										}
										$harga = $produk->detailProduk->first()->harga ?? 0;
									...
									<tr class="table_row">
										<td class="column-1">
											<div class="how-itemcart1">
												<img src="{{ asset('storage/' . $produk->gambar) }}" alt="IMG" style="opacity: 50%">
											</div>
										</td>
										...
										<td class="column-3" style="opacity: 50%">Rp{{ number_format($harga, 0, ',', '.') }}</td>
										<td class="column-5" >
											<a href="#" class="btn btn-sm btn-danger btn-delete-cart" data-slug="{{ $item->slug }}">Hapus</a>
										</td>
									</tr>
								@endforeach
							</table>
				...
				<div class="col-sm-10 col-lg-7 col-xl-5 m-lr-auto m-b-50">
					<form id="form-checkout" action="{{ route('checkout') }}">
						<div class="bor10 p-lr-40 p-t-30 p-b-40 m-l-63 m-r-40 m-lr-0-xl p-lr-15-sm">
							...
							<div class="bor12 p-t-27 p-b-13">
								...
								<div class="p-t-10">
									...
									<input class="form-control m-b-12" placeholder="No.Handphone" name="telepon" type="text" value="">

								</div>
								<div class="bor8 m-b-30">
									<textarea class="stext-111 cl2 plh3 size-120 p-lr-28 p-tb-25" name="alamat_pengiriman" placeholder="Add your address"></textarea>
								</div>
							</div>
